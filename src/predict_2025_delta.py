...
model = xgb.Booster()
model.load_model(model_dir / "xgb_delta_ranker.json")
...
df = df.merge(
    last_driver_form.drop(columns=["season", "round", "race_name"]),
    on="driver_id",
    how="left"
)
...
df = df.merge(
    driver_track.groupby(["driver_id", "race_name"]).tail(1)
...
df = df.merge(
    driver_weather.groupby("driver_id").tail(1)
...
df["predicted_delta"] = model.predict(dtest)

df["predicted_finish_raw"] = df["grid_position"] + df["predicted_delta"]

df = (
    df.sort_values(["round", "predicted_finish_raw"])
...
df.to_csv(processed_dir / "predicted_2025_race_results_delta.csv", index=False)
